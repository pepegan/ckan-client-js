...
class GitLfsBasicTransfer {
  ...
  async _doUpload() {
    ...
    const href_giftless = urlParsed['origin'] + '/giftless' + urlParsed['pathname']
    ...
    return axios.put(href_giftless, body, config);
  }
  ...
  async _doVerify() {
    ...
    const urlParsed = new URL(this._verifyAction.href);
    const href_giftless = urlParsed['origin'] + '/giftless' + urlParsed['pathname']

    return axios.post(href_giftless, body, config)
  }
}
